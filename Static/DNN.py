...
model.add(keras.layers.Dense(input_shape = [len(featureMatrix[0])], units=50, activation="relu",
                              name="hiddenL_1"))
model.add(tf.keras.layers.Dropout(rate =0.15))
model.add(keras.layers.Dense(units=50, activation="relu", name="hiddenL_2"))
model.add(tf.keras.layers.Dropout(rate =0.15))
model.add(keras.layers.Dense(units=50, activation="relu", name="hiddenL_3"))
model.add(tf.keras.layers.Dropout(rate =0.15))
...
history = model.fit(featureMatrixTR, labelVectorTR, batch_size=2048, epochs=500,
...
print(classification_report(labelVector, model_predictions,digits=4))
