...
tf.keras.layers.Dense(input_shape=[len(featureMatrixTR[0])],units =40, name="SAE_Layer_1"),
tf.keras.layers.Dense(units=20, activation="relu", name="SAE_Layer_2_Features"),

])
stacked_decoder = tf.keras.models.Sequential([
tf.keras.layers.Dense(units=40, activation="relu", input_shape=[20], name="SAE_Layer_3"),
...
# Callbacks
cp = tf.keras.callbacks.ModelCheckpoint("SAE_AndMAl_Zero.h5",save_best_only =True, save_freq='epoch')
...
history = stacked_ae.fit(featureMatrixTR, featureMatrixTR,epochs=100, callbacks = [tensorboard_cb, cp], validation_split=0.2)
