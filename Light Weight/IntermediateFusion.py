...
# Callbacks
cp = tf.keras.callbacks.ModelCheckpoint("SAE_HYBRID.h5",save_best_only =True, save_freq='epoch')



def training():
    stacked_ae.fit(featureMatrixTR, featureMatrixTR, batch_size=2048, epochs=500,
                    validation_split=0.2)


history = stacked_ae.fit(featureMatrixTR, featureMatrixTR,epochs=500, callbacks = [cp], validation_split=0.2)

loaded_model = tf.keras.models.load_model("SAE_HYBRID.h5")
...
x = os.stat("SAE_HYBRID.h5").st_size
# Light weight Tests"
