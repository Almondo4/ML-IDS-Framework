...
stackedModel = keras.models.load_model("Combined_MultiSAE_2.h5")
stackedModel.trainable = False
stackedModel._name ="sae_stacked"


modelEncoder = keras.Model(inputs=[stackedModel.input], outputs=[stackedModel.layers[7].output])


DNN = keras.layers.Dense(units=200, activation="relu", name='input2model_DNN', input_shape=[50])(modelEncoder.output)
...
model.compile(optimizer=tf.optimizers.SGD(lr=0.00004),
...
history = model.fit([featureMatrixTRD,featureMatrixTRS], labelVectorTR, batch_size=256, epochs=1500,
