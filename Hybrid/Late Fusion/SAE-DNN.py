...
DNN = keras.layers.Dense(units=100, activation="relu", name='input2model_DNN',kernel_regularizer="l1_l2", input_shape=[100])(modelEncoder.output)
DNN= tf.keras.layers.Dropout(rate =0.25)(DNN)
DNN= keras.layers.Dense(units=100, activation="relu", name='hiddenlayer_DNN')(DNN)
DNN= tf.keras.layers.Dropout(rate =0.25)(DNN)
DNN = keras.layers.Dense(units=100, activation="relu",kernel_regularizer="l1_l2", name='hiddenLayer2_DNN')(DNN)
...
history = model.fit([featureMatrixTRD,featureMatrixTRS], labelVectorTR, batch_size=512, epochs=1500, validation_split=0.2,callbacks=[cp, tensorboard_cb])
