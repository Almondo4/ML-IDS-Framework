...
tf.keras.layers.Dense(input_shape=[len(featureMatrixTR[0])],units =300, name="SAE_Layer_1"),
tf.keras.layers.Dense(units=100, activation="relu", name="SAE_Layer_2_Features"),

])
stacked_decoder = tf.keras.models.Sequential([
tf.keras.layers.Dense(units=300, activation="relu", input_shape=[100], name="SAE_Layer_3"),
...
# Callbacks
cp = tf.keras.callbacks.ModelCheckpoint("SAE_CICMAl.h5",save_best_only =True, save_freq='epoch')
...
history = stacked_ae.fit(featureMatrixTR, featureMatrixTR,epochs=200, callbacks = [tensorboard_cb, cp], validation_split=0.2)
